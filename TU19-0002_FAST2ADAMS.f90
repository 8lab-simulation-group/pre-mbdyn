...
!   WRITE (UnAC,FmtTRT   )  'INTEGRATOR/GSTIFF, ERROR = 0.001, HMAX = ', DT, ', INTERPOLATE = ON'
    WRITE (UnAC,FmtTRT   )  'INTEGRATOR/GSTIFF, ERROR = 0.1'
! --- TU17-0001: End ---
...
! --- TU17-0001: Start ---
! Platform flexibility is enabled.

IF ( ( PtfmModel == 3 ) .AND. ( PtfmLdMod == 1 ) ) THEN

IF ( NPtfmDivH > 0 ) THEN
   WRITE    (UnAC,FmtText  )  'DEACTIVATE/JOINT, RANGE = '//TRIM(Int2LStr(           40300 + 1        ))//', '// &
                                                            TRIM(Int2LStr(           40300 + NPtfmDivH ))
ENDIF
...
! --- TU17-0001: Start ---
REAL(ReKi)                   :: TmpVec6   (6)                                   ! A temporary vector used in various computations.
REAL(ReKi)                   :: TmpVec7   (6)                                   ! A temporary vector used in various computations.
REAL(ReKi)                   :: TmpVec8   (6)                                   ! A temporary vector used in various computations.
REAL(ReKi)                   :: TmpVec9   (6)                                   ! A temporary vector used in various computations.
REAL(ReKi)                   :: TmpMat6   (6,6)                                   ! A temporary vector used in various computations.
REAL(ReKi)                   :: DIS
REAL(ReKi)                   :: MINDIS
REAL(ReKi)                   :: NacCAxn                                         ! Aerodynamic center of Nacelle
REAL(ReKi)                   :: NacCAyn                                         ! Aerodynamic center of Nacelle
REAL(ReKi)                   :: NacCAzn                                         ! Aerodynamic center of Nacelle
REAL(ReKi)                   :: CD_Nacelle                                      ! CD of Nacelle
REAL(ReKi)                   :: An_Nacelle                                      ! Normal area of Nacelle
REAL(ReKi)                   :: NacFx                                           ! Wind Load Fx (kN) acting on Nacelle aedodynamic center
REAL(ReKi)                   :: NacFy                                           ! Wind Load Fy (kN) acting on Nacelle aedodynamic center
REAL(ReKi)                   :: NacFz                                           ! Wind Load Fz (kN) acting on Nacelle aedodynamic center
REAL(ReKi)                   :: RotCAxn                                         ! Aerodynamic center of Rotor
REAL(ReKi)                   :: RotCAyn                                         ! Aerodynamic center of Rotor
REAL(ReKi)                   :: RotCAzn                                         ! Aerodynamic center of Rotor
REAL(ReKi)                   :: RotFx                                           ! Wind Load Fx (kN) acting on Rotor aedodynamic center
REAL(ReKi)                   :: RotFy                                           ! Wind Load Fy (kN) acting on Rotor aedodynamic center
REAL(ReKi)                   :: RotFz                                           ! Wind Load Fz (kN) acting on Rotor aedodynamic center
INTEGER(4)                   :: TmpID1                                          ! A temporary ID for a PART, MARKER, etc...
INTEGER(4)                   :: RefID                                           ! A temporary ID for a PART, MARKER, etc...

INTEGER(4)                   :: UnNac     = 30                                  ! I/O unit number for the Nacelle data input (_Nacelle.ipt)
! --- TU17-0001: End ---
...
IF ( ( PtfmModel == 3 ) .AND. ( PtfmLdMod == 1 ) ) THEN
   ...
   CALL UserPtfmLd( TmpVec6, TmpVec7, TmpVec8, ZTime, DirRoot, TmpMat6, TmpVec9 ) ! add XDD (TmpVec8)
...
WRITE (UnAD,FmtText  )  '!------------------ Ground: Floating Aero and Hydro for Nacelle ------------------------'
   TmpID = 800 + TwrNodes + 1
   WRITE (UnAD,'(A)'       )  '!                             adams_view_name=''Nacelle FloatingAeroHydro_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtText     )  ', PART = 1'
   WRITE (UnAD,FmtText     )  ', FLOATING'

   TmpID = 800 + TwrNodes + 2
   WRITE (UnAD,'(A)'       )  '!                             adams_view_name=''Nacelle FloatingAeroHydro2_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtText     )  ', PART = 1'
   WRITE (UnAD,FmtText     )  ', FLOATING'

   TmpID = 800 + TwrNodes + 3
   WRITE (UnAD,'(A)'       )  '!                             adams_view_name=''Nacelle FloatingAeroHydro3_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtText     )  ', PART = 1'
   WRITE (UnAD,FmtText     )  ', FLOATING'

WRITE (UnAD,FmtText  )  '!------------------ Ground: Floating Hydro for Platform ------------------------'

DO J = 1,NPtfmDivH ! Loop through the platform elements
   TmpID = 900 + J
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PlatformSec', J, 'FloatingHydro_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtText     )  ', PART = 1'
   WRITE (UnAD,FmtText     )  ', FLOATING'
ENDDO
...
J = 1
MINDIS = ABS(EZCOORD(J) + PtfmRef)
DO I = 2,NPtfmDivH
   DIS = EZCOORD(I) + PtfmRef
   IF ( ABS(DIS) < MINDIS ) THEN
       J = I
       MINDIS = ABS(DIS)
   ENDIF
ENDDO
...
WRITE (UnAD,FmtText  )  '!                             adams_view_name=''PlatformRef_M'''
WRITE (UnAD,FmtText  )  'MARKER/1000'
WRITE (UnAD,FmtText  )  ', PART = '//TRIM(Int2LStr( RefID ))
WRITE (UnAD,FmtTRTRTR)  ', QP = ', 0.0 - ( PtfmRef + EZCOORD(J) ), ', ', 0.0, ', ', 0.0
...
WRITE (UnAD,FmtText  )  'MARKER/1005'
WRITE (UnAD,FmtText  )  ', PART = '//TRIM(Int2LStr( RefID ))
WRITE (UnAD,FmtTRTRTR)  ', QP = ', rZYzt - ( PtfmRef + EZCOORD(J) ), ', ', 0.0, ', ', 0.0
WRITE (UnAD,FmtText  )  ', REULER = 90D, 90D, 90D'

WRITE (UnAD,FmtText  )  '                             adams_view_name=''UndeflTwrTop_M'''
WRITE (UnAD,FmtText  )  'MARKER/1030'
WRITE (UnAD,FmtText  )  ', PART = 40001'
WRITE (UnAD,FmtTRTRTR)  ', QP = ', RefTwrHt - ( PtfmRef + EZCOORD(1) ), ', ', 0.0, ', ', 0.0 
WRITE (UnAD,FmtText  )  ', REULER = 90D, 90D, 90D'
...
WRITE (UnAD,FmtText  )  '!------------------------------ Support Platform (section models) --------------'
...
DO J = 1,NPtfmDivH ! Loop through the platform elements
   ...
   TmpID = 40000 + J
   TmpID2 = 40500 + J
   ThnBarI = EMASS(J)*( ELENGTH(J)**3 )/12.0 ! Define the transverse inertias of the platform element (both identical) to be that of a thin uniform bar.
   WRITE (UnAD,'(A,I2.2,A)')  '!--------------------- Support Platform: Section ', J, ' ---------------------------'
   TmpVec  = PtfmSurge*z1 + PtfmHeave*z2 - PtfmSway *z3 + ( PtfmRef + EZCOORD(J) )*a2 ! rT = Position vector from ground to current platform node (point T(J))
   TmpVec1 = TmpVec + (/1.0, 0.0, 0.0/)
   TmpVec2 = TmpVec + (/0.0, 1.0, 0.0/)
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, '_P'''
   WRITE (UnAD,FmtText     )  'PART/'//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtTRTRTR   )  ', QG = ', TmpVec (1), ', ', -TmpVec (3), ', ', TmpVec (2)  ! Orient the platform
   ...
   WRITE (UnAD,FmtTR       )  ', MASS = ', EMASS(J)*ELENGTH(J) + SmllNmbr
! --- TU19-0002
!   WRITE (UnAD,FmtTRTRTR   )  ', IP = ', ( ( InerPFA(J) + InerPSS(J) )*ELENGTH(J) ) + SmllNmbr, ', ', &
!                              ( InerPFA(J)*ELENGTH(J) ) + ThnBarI + SmllNmbr, ', ', ( InerPSS(J)*ELENGTH(J) ) + ThnBarI + SmllNmbr
   WRITE (UnAD,FmtTRTRTR   )  ', IP = ', ( ( InerPIP(J) )*ELENGTH(J) ) + SmllNmbr, ', ', &
                              ( InerPFA(J)*ELENGTH(J) ) + ThnBarI + SmllNmbr, ', ', ( InerPSS(J)*ELENGTH(J) ) + ThnBarI + SmllNmbr
! --- TU19-0002

   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, 'Elastic_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtText     )  ', PART = '//TRIM(Int2LStr( TmpID ))
   ...
   TmpID2 = 40800 + J
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, 'AeroHydro_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', PART = '//TRIM(Int2LStr( TmpID ))
   ...
   TmpID2 = 40200 + J
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, 'Graphics_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', PART = '//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtTRTRTR   )  ', QP = ', -0.5*ELENGTH(J), ', ', 0.0, ', ', 0.0
   WRITE (UnAD,FmtText     )  ', REULER = 90D, 90D, 90D'

   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, '_G'''
   WRITE (UnAD,FmtText     )  'GRAPHICS/'//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', CYLINDER'
   WRITE (UnAD,FmtText     )  ', CM = '//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtTR       )  ', LENGTH = ', ELENGTH(J)
   WRITE (UnAD,FmtText     )  ', SIDES = '//TRIM(Int2LStr( NSides ))
   WRITE (UnAD,FmtTR       )  ', RADIUS = ', EDIAMETER(J)*0.5
   WRITE (UnAD,FmtText     )  ', SEG = '//TRIM(Int2LStr( NSides ))
   ...
   TmpID2 = 40300 + J
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, 'ToPtfmSecBelow_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', PART = '//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtTRTRTR   )  ', QP = ', -0.5*ELENGTH(J), ', ', 0.0, ', ', 0.0
   WRITE (UnAD,FmtText     )  ', REULER = 0D, 0D, 0D'
   ...
   TmpID2 = 40400 + J
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, 'ToPtfmSecAbove_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', PART = '//TRIM(Int2LStr( TmpID ))
   WRITE (UnAD,FmtTRTRTR   )  ', QP = ', 0.5*ELENGTH(J), ', ', 0.0, ', ', 0.0
   WRITE (UnAD,FmtText     )  ', REULER = 0D, 0D, 0D'
   ...
   TmpID2 = 40500 + J
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, 'CM_M'''
   WRITE (UnAD,FmtText     )  'MARKER/'//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', PART = '//TRIM(Int2LStr( TmpID ))
...
WRITE (UnAD,FmtText  )  'MARKER/1100'  ! MARKER/1100 is equivalent to the tower-base coordinate system: X = Xt, Y = Yt, Z = Zt
WRITE (UnAD,FmtText  )  ', PART = 40001'
WRITE (UnAD,FmtTRTRTR)  ', QP = ', PtfmRef - TwrDraft - ( PtfmRef + EZCOORD(1) ), ', ', 0.0, ', ', 0.0
WRITE (UnAD,FmtText  )  ', REULER = 90D, 90D, 90D'
...
WRITE (UnAD,FmtText  )  'MARKER/1400'
WRITE (UnAD,FmtText  )  ', PART = 40001'
WRITE (UnAD,FmtTRTRTR)  ', QP = ', rZT0zt - ( PtfmRef + EZCOORD(1) ), ', ', 0.0, ', ', 0.0
WRITE (UnAD,FmtText  )  ', REULER = 0D, 0D, 0D'
...
CALL OpenFInpFile ( UnNac, TRIM( RootName )//'_Nacelle.ipt' )

READ (UnNac,*) NacCAxn
READ (UnNac,*) NacCAyn
READ (UnNac,*) NacCAzn
READ (UnNac,*) CD_Nacelle
READ (UnNac,*) An_Nacelle
READ (UnNac,*) NacFx       ! TU11-00090
READ (UnNac,*) NacFy       ! TU11-00090
READ (UnNac,*) NacFz       ! TU11-00090
READ (UnNac,*) RotCAxn     ! TU11-00090
READ (UnNac,*) RotCAyn     ! TU11-00090
READ (UnNac,*) RotCAzn     ! TU11-00090
READ (UnNac,*) RotFx       ! TU11-00090
READ (UnNac,*) RotFy       ! TU11-00090
READ (UnNac,*) RotFz       ! TU11-00090

CLOSE (UnNac)

WRITE (UnAD,FmtText  )  '!                             adams_view_name=''NacelleAero_M'''
WRITE (UnAD,FmtText  )  'MARKER/2006'
WRITE (UnAD,FmtText  )  ', PART = 2000'
WRITE (UnAD,FmtTRTRTR)  ', QP = ', NacCAxn, ', ', NacCAyn, ', ', NacCAzn
WRITE (UnAD,FmtText  )  ', REULER = 0, 0, 0'

WRITE (UnAD,FmtText  )  '!                             adams_view_name=''NacelleAero2_M'''
WRITE (UnAD,FmtText  )  'MARKER/2007'
WRITE (UnAD,FmtText  )  ', PART = 2000'
WRITE (UnAD,FmtTRTRTR)  ', QP = ', RotCAxn, ', ', RotCAyn, ', ', RotCAzn
WRITE (UnAD,FmtText  )  ', REULER = 0, 0, 0'
...
DO J = 1,NPtfmDivH-1 ! Loop through the platform nodes/elements

   TmpID  = 40400 + J + 1   ! ID of the MARKER on the top    of platform element J + 1.
   TmpID2 = 40300 + J       ! ID of the MARKER on the bottom of platform element J.
   WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmSec', J, 'ToPtfmSecBelow_J'''
   WRITE (UnAD,FmtText     )  'JOINT/'//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', FIXED'
   WRITE (UnAD,FmtText     )  ', I = '//TRIM(Int2LStr( TmpID2 ))
   WRITE (UnAD,FmtText     )  ', J = '//TRIM(Int2LStr( TmpID  ))
...
WRITE (UnAD,FmtText  )  '!------------------------ Platform Stiffness and Damping -----------------------'
...
DO J = 2,NPtfmDivH ! Loop through all but the innermost tower nodes/elements

   TmpID  = 40000 + J                               ! ID of the MARKER in the middle of tower element J.
   TmpID2 = 40000 + J - 1                           ! ID of the MARKER in the middle of tower element J - 1.
   TmpLength  = 0.5*( ELENGTH(J) + ELENGTH(J-1) )  ! Distance between tower node J and node J-1.
   ...
   KMatrix(1,1) =  0.001*0.5*(     StiffPEA(J) + StiffPEA(J-1) )/TmpLength    ! Use the same KMatrix specified in the ADAMS/WT user's guide.
   KMatrix(2,2) =  0.001*6.0*(     StiffPSS(J) + StiffPSS(J-1) )/TmpLength3
   KMatrix(3,3) =  0.001*6.0*(     StiffPFA(J) + StiffPFA(J-1) )/TmpLength3
   KMatrix(4,4) =  0.001*0.5*(     StiffPGJ(J) + StiffPGJ(J-1) )/TmpLength
   KMatrix(5,5) =  0.001*    ( 3.0*StiffPFA(J) + StiffPFA(J-1) )/TmpLength
   KMatrix(6,6) =  0.001*    ( 3.0*StiffPSS(J) + StiffPSS(J-1) )/TmpLength
   KMatrix(2,6) = -0.001*2.0*( 2.0*StiffPSS(J) + StiffPSS(J-1) )/TmpLength2
   KMatrix(3,5) =  0.001*2.0*( 2.0*StiffPFA(J) + StiffPFA(J-1) )/TmpLength2
   ...
   WRITE (UnAD,'(A,I2.2,A)'   )  '!                             adams_view_name=''PtfmSec', J, 'ToPtfmSecAbove_F'''
...
ENDDO             ! J - Tower nodes/elements
      ...
      WRITE (UnAD,FmtText  )  '!------------------------------- Platform Loading ------------------------------'
      DO J = 1,NPtfmDivH ! Loop through the platform nodes/elements

         TmpID  = 40800 + J       ! ID of RM marker.
         TmpID2 = 900 + J         ! ID of floating marker.
         WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''PtfmLoadingSec', J,''''
         WRITE (UnAD,FmtText  )  'GFORCE/'//TRIM(Int2LStr( TmpID ))
         WRITE (UnAD,FmtText  )  ', I = '//TRIM(Int2LStr( TmpID ))
         WRITE (UnAD,FmtText  )  ', JFLOAT = '//TRIM(Int2LStr( TmpID2  ))
         WRITE (UnAD,FmtText  )  ', RM = '//TRIM(Int2LStr( TmpID ))
         WRITE (UnAD,FmtText  )  ', FUNCTION = USER( 0 )'
...
IF ( ( PtfmModel == 3 ) .AND. ( PtfmLdMod == 1 ) ) THEN
      ...
      DO J = 1,TwrNodes ! Loop through the platform nodes/elements

         TmpID  = 1100 + J       ! ID of the MARKER on the top    of tower element J - 1.
         TmpID1 = 1800 + J       ! ID of RM marker.
         TmpID2 = 800 + J        ! ID of floating marker.
         WRITE (UnAD,'(A,I2.2,A)')  '!                             adams_view_name=''TwrLoadingSec', J,''''
         WRITE (UnAD,FmtText  )  'GFORCE/'//TRIM(Int2LStr( TmpID ))
         WRITE (UnAD,FmtText  )  ', I = '//TRIM(Int2LStr( TmpID1 ))
         WRITE (UnAD,FmtText  )  ', JFLOAT = '//TRIM(Int2LStr( TmpID2  ))
         WRITE (UnAD,FmtText  )  ', RM = '//TRIM(Int2LStr( TmpID1 ))
         WRITE (UnAD,FmtText  )  ', FUNCTION = USER( 0 )'

      ENDDO             ! J - platform nodes/elements

      J = TwrNodes + 1 ! Node number for Nacelle
      TmpID  = 2006           ! ID of the MARKER on the Nacelle Aerodynamic marker.
      TmpID1 = 2006           ! ID of RM marker.
      TmpID2 = 800 + J        ! ID of floating marker.
      WRITE (UnAD,'(A)'    )  '!                             adams_view_name=''NacelleLoading'''
      WRITE (UnAD,FmtText  )  'GFORCE/'//TRIM(Int2LStr( TmpID ))
      WRITE (UnAD,FmtText  )  ', I = '//TRIM(Int2LStr( TmpID1 ))
      WRITE (UnAD,FmtText  )  ', JFLOAT = '//TRIM(Int2LStr( TmpID2  ))
      WRITE (UnAD,FmtText  )  ', RM = '//TRIM(Int2LStr( TmpID1 ))
      WRITE (UnAD,FmtText  )  ', FUNCTION = USER( '//TRIM(Flt2LStr( CD_Nacelle ))//', '//TRIM(Flt2LStr( An_Nacelle ))//' )'

      J = TwrNodes + 1 ! Node number for Nacelle
      TmpID  = 2007           ! ID of the MARKER on the Nacelle Aerodynamic marker.
      TmpID1 = 2006           ! ID of RM marker.
      TmpID2 = 800 + J + 1    ! ID of floating marker.
      WRITE (UnAD,'(A)'    )  '!                             adams_view_name=''NacelleLoading2'''
      WRITE (UnAD,FmtText  )  'GFORCE/'//TRIM(Int2LStr( TmpID ))
      WRITE (UnAD,FmtText  )  ', I = '//TRIM(Int2LStr( TmpID1 ))
      WRITE (UnAD,FmtText  )  ', JFLOAT = '//TRIM(Int2LStr( TmpID2  ))
      WRITE (UnAD,FmtText  )  ', RM = '//TRIM(Int2LStr( TmpID1 ))
      WRITE (UnAD,FmtText  )  ', FX = '//TRIM(Flt2LStr( NacFx ))//'\'
      WRITE (UnAD,FmtText  )  ', FY = '//TRIM(Flt2LStr( NacFy ))//'\'
      WRITE (UnAD,FmtText  )  ', FZ = '//TRIM(Flt2LStr( NacFz ))//'\'
      WRITE (UnAD,FmtText  )  ', TX = '//TRIM(Flt2LStr( 0.0 ))//'\'
      WRITE (UnAD,FmtText  )  ', TY = '//TRIM(Flt2LStr( 0.0 ))//'\'
      WRITE (UnAD,FmtText  )  ', TZ = '//TRIM(Flt2LStr( 0.0 ))

      J = TwrNodes + 1 ! Node number for Nacelle
      TmpID  = 2008           ! ID of the MARKER on the Nacelle Aerodynamic marker.
      TmpID1 = 2007           ! ID of RM marker.
      TmpID2 = 800 + J + 2    ! ID of floating marker.
      WRITE (UnAD,'(A)'    )  '!                             adams_view_name=''NacelleLoading3'''
      WRITE (UnAD,FmtText  )  'GFORCE/'//TRIM(Int2LStr( TmpID ))
      WRITE (UnAD,FmtText  )  ', I = '//TRIM(Int2LStr( TmpID1 ))
      WRITE (UnAD,FmtText  )  ', JFLOAT = '//TRIM(Int2LStr( TmpID2  ))
      WRITE (UnAD,FmtText  )  ', RM = '//TRIM(Int2LStr( TmpID1 ))
      WRITE (UnAD,FmtText  )  ', FX = '//TRIM(Flt2LStr( RotFx ))//'\'
      WRITE (UnAD,FmtText  )  ', FY = '//TRIM(Flt2LStr( RotFy ))//'\'
      WRITE (UnAD,FmtText  )  ', FZ = '//TRIM(Flt2LStr( RotFz ))//'\'
      WRITE (UnAD,FmtText  )  ', TX = '//TRIM(Flt2LStr( 0.0 ))//'\'
      WRITE (UnAD,FmtText  )  ', TY = '//TRIM(Flt2LStr( 0.0 ))//'\'
      WRITE (UnAD,FmtText  )  ', TZ = '//TRIM(Flt2LStr( 0.0 ))
